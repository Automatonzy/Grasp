...
class SubsetRetrievePipeline:
    def __init__(self, subset_dir, save_root='./', topk=5, lang_mode='clip', crop=False, data_source=None) -> None:
        ...
        task_root_dir = os.path.join(subset_dir, "database")
        task_list = os.listdir(task_root_dir)
        ...
        # 预处理所有任务的特征
        print("Scanning available preprocessed tasks...")
        self.preprocessed_data = {}
        for task in tqdm(self.task_list):
            try:
                task_name = task.replace(" ", "_")
                task_dir = os.path.join(subset_dir, "database", task_name)
                pkl_path = os.path.join(task_dir, task_name + "_new.pkl")
                
                if os.path.exists(pkl_path):
                    with open(pkl_path, 'rb') as f:
                        retrieved_data_dict = pickle.load(f)
                    self.preprocessed_data[task] = retrieved_data_dict
                else:
                    print(f"[⚠️] No .pkl found for task {task}")
            except Exception as e:
                print(f"[❌] Error reading task {task}: {str(e)}")
                traceback.print_exc()     
    ...
    def imd_ranking(self, sorted_retrieved_data_dict, obj_prompt):
        ...
        for i in range(self.topk):
            ...
            curr = sorted_index[i]
            for k in ["img", "traj", "masked_img", "mask", "direction", "masked_img_feat","masked_img_mapper"]:
    ...
    def retrieve(self, current_task, current_obs, log=True):
        if log:
            print("<1> Retrieve the most similar task")

        retrieved_task = self.language_retrieve(current_task)
        print(f"[DEBUG] 已加载任务 keys: {list(self.preprocessed_data.keys())}")
        print(f"[DEBUG] 当前 retrieved_task: {retrieved_task}")

        # === 尝试动态加载 missing 任务 ===
        if retrieved_task not in self.preprocessed_data:
            print(f"[⚠️] '{retrieved_task}' 不在缓存中，尝试现场加载...")

            task_name = retrieved_task.replace(" ", "_")
            task_dir = os.path.join(self.subset_dir, "database", task_name)
            pkl_path = os.path.join(task_dir, task_name + "_new.pkl")

            if not os.path.exists(pkl_path):
                raise FileNotFoundError(f"未找到任务 '{retrieved_task}' 的缓存文件: {pkl_path}")

            with open(pkl_path, 'rb') as f:
                retrieved_data_dict = pickle.load(f)
            self.last_retrieved_pkl_path = pkl_path  # ✅ 设置路径供写回使用
            # 若缺少 masked_img_feat，现场提取
            if "masked_img_feat" not in retrieved_data_dict or "masked_img_mapper" not in retrieved_data_dict:
                if "masked_img_feat" not in retrieved_data_dict:
                    retrieved_data_dict['masked_img_feat'] = []
                if "masked_img_mapper" not in retrieved_data_dict:
                    retrieved_data_dict['masked_img_mapper'] = []
                for img, mask in zip(retrieved_data_dict['masked_img'], retrieved_data_dict['mask']):
                    feat, mapper= extract_ft(
                        Image.fromarray(img).convert("RGB"),
                        prompt=f"A photo of object",
                        ftype='sd',
                        featurizer=self.sd_featurizer,
                        mask=mask
                    )
                    retrieved_data_dict['masked_img_feat'].append(feat.cpu())
                    retrieved_data_dict['masked_img_mapper'].append(mapper.to_dict())

                # ✅ 写回磁盘前进行检查
                if isinstance(self.last_retrieved_pkl_path, str) and os.path.exists(os.path.dirname(self.last_retrieved_pkl_path)):
                    with open(self.last_retrieved_pkl_path, "wb") as f:
                        pickle.dump(retrieved_data_dict, f)
                    print(f"[✔] 已保存预处理结果至 {self.last_retrieved_pkl_path}")
                else:
                    print("⚠️ 无法保存回 .pkl 文件：last_retrieved_pkl_path 未设置或路径无效")

            self.preprocessed_data[retrieved_task] = retrieved_data_dict

        # === 开始正常流程 ===
        retrieved_data_dict = self.preprocessed_data[retrieved_task]
        # ✅ 若 last_retrieved_pkl_path 尚未设置（即任务来自缓存），我们主动推断它
        if self.last_retrieved_pkl_path is None:
            task_name = retrieved_task.replace(" ", "_")
            task_dir = os.path.join(self.subset_dir, "database", task_name)
            pkl_path = os.path.join(task_dir, task_name + "_new.pkl")
            if os.path.exists(pkl_path):
                self.last_retrieved_pkl_path = pkl_path
        obj_name = "object"
        ...
        if "masked_img_feat" not in retrieved_data_dict.keys() or "masked_img_mapper" not in retrieved_data_dict.keys():
            if log: 
                print("[✔] 提前提取 StableDiffusion 特征...")
                if "masked_img_feat" not in retrieved_data_dict:
                    retrieved_data_dict['masked_img_feat'] = []
                if "masked_img_mapper" not in retrieved_data_dict:
                    retrieved_data_dict['masked_img_mapper'] = []
            ...
            for img, mask in zip(retrieved_data_dict['masked_img'], retrieved_data_dict['mask']):
                ...
                retrieved_data_dict['masked_img_feat'].append(feat.cpu())
                retrieved_data_dict['masked_img_mapper'].append(mapper.to_dict())  # 转 CPU 避免显存占用过多
        ...
        if not topk_retrieved_data_dict["img"]:
            ...
            top1_retrieved_data_dict = {
                ...
                "masked_img_feat": None,
                "masked_img_mapper":None
            }
        else:
            top1_retrieved_data_dict = {
                "query_img": current_obs,
                ...
                "masked_img_feat": topk_retrieved_data_dict["masked_img_feat"][top1_idx],
                "masked_img_mapper": topk_retrieved_data_dict["masked_img_mapper"][top1_idx]
            }
        end_time2 = time.time()
