...
def get_distance_imd(feature1, feature2, mask1, mask2, RESOLUTION=64):
    ...
    # 处理掩码
    mask1 = torch.from_numpy(mask1[..., 0]).float().to(device)
    mask2 = torch.from_numpy(mask2[..., 0]).float().to(device)

    mask1 = F.interpolate(mask1[None, None], size=(RESOLUTION, RESOLUTION), mode='nearest').squeeze(0).squeeze(0)  # [H, W]
    mask2 = F.interpolate(mask2[None, None], size=(RESOLUTION, RESOLUTION), mode='nearest').squeeze(0).squeeze(0)  # [H, W]

    # 插值到统一尺寸
    feat1 = F.interpolate(feature1, size=(RESOLUTION, RESOLUTION), mode='bilinear').squeeze(0)  # [c, H, W]
    feat2 = F.interpolate(feature2, size=(RESOLUTION, RESOLUTION), mode='bilinear').squeeze(0)  # [c, H, W]

    # 掩码特征（不再 repeat，只 unsqueeze）
    feat1 = feat1 * mask1.unsqueeze(0)
    feat2 = feat2 * mask2.unsqueeze(0)

    # 将特征展平为 [N, c]
    feat1_flat = feat1.permute(1, 2, 0).reshape(-1, c)  # [N, c]
    feat2_flat = feat2.permute(1, 2, 0).reshape(-1, c)  # [N, c]
    mask1_flat = mask1.reshape(-1) > 0  # [N]
    mask2_flat = mask2.reshape(-1) > 0  # [N]

    # 提取有效区域
    feat1_valid = feat1_flat[mask1_flat]  # [N1, c]
    feat2_valid = feat2_flat[mask2_flat]  # [N2, c]

    if feat1_valid.size(0) == 0 or feat2_valid.size(0) == 0:
        return torch.tensor(1e6, device=device)  # fallback: 无有效区域

    # 计算所有源 patch 与目标 patch 的 L2 距离
    # feat1_valid: [N1, c], feat2_valid: [N2, c]
    dists = torch.cdist(feat1_valid, feat2_valid, p=2)  # [N1, N2]
    min_per_src = torch.min(dists, dim=1)[0]  # 每个 src patch 找最近的 tgt patch

    return min_per_src.mean()  # 平均最小距离
