...
def inference_one_image(cam_image, model, predictor, box_threshold, text_threshold,
                        text_prompt, device, point_prompt=None):
    """
    使用 Grounded-SAM 对图像进行目标分割，支持 point_prompt 或 text_prompt 模式。
    """
    # Step 1: 设置图像（自动做 resize 和 transform）
    predictor.set_image(cam_image)
    height, width = cam_image.shape[:2]
    print(f"[Debug] 原始图像尺寸: (H={height}, W={width})")

    # Step 2: 使用点提示
    if point_prompt is not None:
        u, v = point_prompt
        print(f"[Debug] 原始提示点: (u={u}, v={v})")

        # ✅ 用 numpy 传入 apply_coords
        orig_coords_np = np.array([[u, v]], dtype=np.float32)
        transformed_coords_np = predictor.transform.apply_coords(orig_coords_np, (height, width))
        transformed_coords = torch.tensor(transformed_coords_np, dtype=torch.float32, device=device).unsqueeze(0)
        ...
        if 0 <= v < mask_np.shape[0] and 0 <= u < mask_np.shape[1]:
            print(f"[Debug] 掩码中提示点 ({u},{v}) 的值: {mask_np[v, u]}")
        else:
            print(f"[Warning] 提示点 ({u},{v}) 超出掩码范围 {mask_np.shape}")
    ...
    # Step 3: 使用文本提示
        ...
        from vision.GroundedSAM.grounded_sam_utils import load_image_from_array, get_grounding_output
        image_pil, image_tensor = load_image_from_array(cam_image)
        ...
        transformed_boxes = predictor.transform.apply_boxes_torch(
            boxes_filt.cpu(), cam_image.shape[:2]
        ).to(device)
